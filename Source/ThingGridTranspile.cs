...
{
    public class ThingGrid_Transpile
    {

        public static IEnumerable<CodeInstruction> RegisterInCell(IEnumerable<CodeInstruction> instructions, ILGenerator iLGenerator)
        {
        ...
        public static IEnumerable<CodeInstruction> ThingAt(IEnumerable<CodeInstruction> instructions, ILGenerator iLGenerator)
        {
            LocalBuilder monitorLockObject = iLGenerator.DeclareLocal(typeof(List<Thing>));
            LocalBuilder lockTaken = iLGenerator.DeclareLocal(typeof(bool));
            List<CodeInstruction> instructionsList = instructions.ToList();
            ...
            {
                if (
                    instructionsList[i + 2].opcode == OpCodes.Callvirt && (MethodInfo)instructionsList[i + 2].operand == AccessTools.Method(typeof(List<Thing>), "get_Item")
                    )
                {
