...
namespace RimThreaded
{
    ...
    public class PawnDestinationReservationManager_Patch
    {
        ...
        public static bool GetPawnDestinationSetFor(PawnDestinationReservationManager __instance, ref PawnDestinationSet __result, Faction faction)
        {
            lock (__instance)
            {
                if (!reservedDestinationsFieldRef(__instance).TryGetValue(faction, out PawnDestinationSet value))
                {
                    value = new PawnDestinationSet();
                    reservedDestinationsFieldRef(__instance).Add(faction, value);
                }
                __result = value;
            }
        ...
        public static bool Notify_FactionRemoved(PawnDestinationReservationManager __instance, Faction faction)
        {
            lock (__instance)
            {
                if (reservedDestinationsFieldRef(__instance).ContainsKey(faction))
                {
                    Dictionary<Faction, PawnDestinationSet> newReservedDestinations = new Dictionary<Faction, PawnDestinationSet>(reservedDestinationsFieldRef(__instance));
                    newReservedDestinations.Remove(faction);
                    reservedDestinationsFieldRef(__instance) = newReservedDestinations;
                }
            }
            return false;
        }
