        ...
        public static IEnumerable<CodeInstruction> OnIntervalPassed(IEnumerable<CodeInstruction> instructions, ILGenerator iLGenerator)
        {
			...
			while (i < instructionsList.Count)
			{
				if (i + 1 < instructionsList.Count &&
					...
					(MethodInfo)instructionsList[i + 1].operand == AccessTools.Method(typeof(GenTemperature), "ComfortableTemperatureRange")
					) {
					...
					yield return new CodeInstruction(OpCodes.Ldc_R4, 10);
